...
describe('ExclamationMarkRule', () => {
  it('should validate when input contains !:', () => {
    const rule = new ExclamationMarkRule(RuleConfigSeverity.Error, 'always');
  ...
  it('should fix by removing ! before : when applicable is never', () => {
    const rule = new ExclamationMarkRule(RuleConfigSeverity.Error, 'never');
    expect(rule.fix('feat!: add new feature')).to.equal('feat: add new feature');
  });
  ...
  it('should handle check method correctly', () => {
    const alwaysRule = new ExclamationMarkRule(RuleConfigSeverity.Error, 'always');
    ...
    const neverRule = new ExclamationMarkRule(RuleConfigSeverity.Error, 'never');
