...
describe('MaxLengthRule', () => {
  ...
  describe('constructor', () => {
    it('should create a rule with the correct parameters', () => {
      assert.doesNotThrow(() => new MaxLengthRule(RuleConfigSeverity.Error, 'always', 10));
    });
  });
  ...
  describe('validate', () => {
    it('should validate text within max length', () => {
      const rule = new MaxLengthRule(RuleConfigSeverity.Error, 'always', 10);
      assert.strictEqual(rule.validate('Hello'), true);
    });

    it('should invalidate text exceeding max length', () => {
      const rule = new MaxLengthRule(RuleConfigSeverity.Error, 'always', 5);
  ...
  describe('check', () => {
    ...
    it('should handle never applicable correctly', () => {
      const rule = new MaxLengthRule(RuleConfigSeverity.Error, 'never', 5);
    ...
    it('should return error object when level is WARNING', () => {
      const rule = new MaxLengthRule(RuleConfigSeverity.Warning, 'always', 5);
      const result = rule.check('Hello World');
    ...
    it('should throw error when level is ERROR', () => {
      ...
      const rule = new MaxLengthRule(RuleConfigSeverity.Error, 'never', 10);
