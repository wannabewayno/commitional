...
export abstract class BaseRule {
   ...
   * @param context Optional git context for file-aware rules
   * @returns Array where null = valid, string = error message
   */
  abstract validate(parts: string[], context?: GitContext): null | Record<number, string>;
   ...
   */
  abstract fix(parts: string[], context?: GitContext): [errors: null | Record<number, string>, fixed: string[]];
  ...
  check(
    ...
    // fix or validate
    const [err, fixed] = fix ? this.fix(input, context) : [this.validate(input, context)];
