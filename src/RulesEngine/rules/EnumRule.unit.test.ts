...
describe('EnumRule', () => {
  ...
  it('should not validate when input is not in the allowed values', () => {
    const rule = new EnumRule('subject', RuleConfigSeverity.Error, 'always', allowedValues);
    expect(rule.validate(['build'])).to.deep.equal({ 0: "the subject can only be one of: 'feat', 'fix', 'docs', 'style' or 'refactor'" });
    expect(rule.validate(['unknown'])).to.deep.equal({ 0: "the subject can only be one of: 'feat', 'fix', 'docs', 'style' or 'refactor'" });
  });
